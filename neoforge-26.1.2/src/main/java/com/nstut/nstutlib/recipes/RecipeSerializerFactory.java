...
public class RecipeSerializerFactory<T extends ModRecipe<T> & RecipeFactory<T>> {
    ...
    private static final MapCodec<ModRecipeData> DATA_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            ...
            FluidStackTemplate.CODEC.listOf().optionalFieldOf("fluidInputs", List.of())
                    .forGetter(data -> Arrays.asList(data.getFluidIngredientTemplates())),
            FluidStackTemplate.CODEC.listOf().optionalFieldOf("fluidOutputs", List.of())
                    .forGetter(data -> Arrays.asList(data.getFluidOutputTemplates())),
    ...
    private static void validate(ModRecipeData data, String recipeId) {
        ...
        for (IngredientItem input : data.getIngredientItems()) {
            ItemStackTemplate template = input == null ? null : input.getItemStackTemplate();
            if (template == null || template.count() <= 0) {
                throw new IllegalArgumentException(recipeId + " has an invalid item input");
            }
        }
        for (OutputItem output : data.getOutputItems()) {
            ItemStackTemplate template = output == null ? null : output.getItemStackTemplate();
            if (template == null || template.count() <= 0) {
        ...
        for (FluidStackTemplate fluid : data.getFluidIngredientTemplates()) {
            if (fluid == null || fluid.amount() <= 0) {
                throw new IllegalArgumentException(recipeId + " has an invalid fluid input");
            }
        }
        for (FluidStackTemplate fluid : data.getFluidOutputTemplates()) {
            if (fluid == null || fluid.amount() <= 0) {
